...
template <>
struct SHA2<224> : private SHA2<256> {
  static constexpr size_t hashsize = 28;
  inline SHA2() {
    sha224_override();
  }
  template <typename... T>
  ...
  void sha224_override();
  inline operator std::array<uint8_t, hashsize>() const {
    std::array<uint8_t, SHA2<256>::hashsize> hash = *((SHA2<256>*)this);
    std::array<uint8_t, hashsize> hashcopy;
    std::copy(hash.begin(), hash.begin() + hashsize, hashcopy.begin());
    return hashcopy;
  }
  std::array<uint8_t, hashsize> data() const { return *this; }
  using SHA2<256>::operator std::string;
  using SHA2<256>::add;
  static inline std::vector<uint8_t> getAsn1Id() {
    return std::initializer_list<uint8_t>{ 0x30, 0x2d, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x04, 0x05, 0x00, 0x04, 0x1c };
  }
  static size_t MaxLength() {
    return (1ULL << 61) - 1;
  }
...
struct SHA2<384> : private SHA2<512> {
  ...
  std::array<uint8_t, hashsize> data() const { return *this; }
  using SHA2<512>::operator std::string;
  using SHA2<512>::add;
  static inline std::vector<uint8_t> getAsn1Id() {
