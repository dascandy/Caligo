...
void SHA2<256>::add(std::span<const uint8_t> data) {
...
void SHA2<512>::add(std::span<const uint8_t> data) {
  size_t offset = msglength % (2*hashsize);
  size_t inoffset = 0;
  msglength += data.size();
  if (offset) {
