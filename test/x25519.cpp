...
TEST_CASE("Single x25519", "[X25519]") {
  s2::vector<uint8_t> rk_1 = { 0x42, 0x2c, 0x8e, 0x7a, 0x62, 0x27, 0xd7, 0xbc, 0xa1, 0x35, 0x0b, 0x3e, 0x2b, 0xb7, 0x27, 0x9f, 0x78, 0x97, 0xb8, 0x7b, 0xb6, 0x85, 0x4b, 0x78, 0x3c, 0x60, 0xe8, 0x03, 0x11, 0xae, 0x30, 0x79 };
  ec_value k = {9};
  ec_value u = {9};
  ec_value new_u = k;
  k = X25519(k, u);
  ...
  ec_value k_1{s2::span<const uint8_t>(rk_1)};
  size_t n = 1;
  CHECK(k == k_1);
}

TEST_CASE("Repeated x25519", "[X25519]") {
  s2::vector<uint8_t> rk_1k = { 0x68, 0x4c, 0xf5, 0x9b, 0xa8, 0x33, 0x09, 0x55, 0x28, 0x00, 0xef, 0x56, 0x6f, 0x2f, 0x4d, 0x3c, 0x1c, 0x38, 0x87, 0xc4, 0x93, 0x60, 0xe3, 0x87, 0x5f, 0x2e, 0xb9, 0x4d, 0x99, 0x53, 0x2c, 0x51 };
  ec_value k_1k{s2::span<const uint8_t>(rk_1k)};
  ec_value k = {9};
  ec_value u = {9};
...
/*
  // Takes 15 minutes
  s2::vector<uint8_t> rk_1M = { 0x7c, 0x39, 0x11, 0xe0, 0xab, 0x25, 0x86, 0xfd, 0x86, 0x44, 0x97, 0x29, 0x7e, 0x57, 0x5e, 0x6f, 0x3b, 0xc6, 0x01, 0xc0, 0x88, 0x3c, 0x30, 0xdf, 0x5f, 0x4d, 0xd2, 0xd2, 0x4f, 0x66, 0x54, 0x24 };
  ec_value k_1M{s2::span<uint8_t>(rk_1M)};
  for (; n < 1000000; n++) {
    ec_value new_u = k;
    k = X25519(k, u);
    u = new_u;
