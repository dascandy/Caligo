...
0x30, 0x82, 0x05, 0x6b, 
  ...
  Caligo::x509certificate c = Caligo::parseCertificate(cert, Caligo::DataFormat::Der);
...
TEST_CASE("Can parse PEM Lets Encrypt root cert") {
  ...
  Caligo::x509certificate c = Caligo::parseCertificate(cert, Caligo::DataFormat::Pem);
...
TEST_CASE("RSA parsed from private key file works symmetrically") {
  ...
  std::unique_ptr<Caligo::PrivateKey> pk = Caligo::parsePrivateKey(std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(privkey.data()), privkey.size()), Caligo::DataFormat::Pem);
  rsa_public_key<4096> pubk(((Caligo::RsaPrivateKey*)(pk.get()))->privkey.n, 65537);
  std::unique_ptr<Caligo::PublicKey> pubkey = std::make_unique<Caligo::RsaPubkey>(pubk);

  std::vector<uint8_t> message;
  message.resize(4096);
  generate_random(message);
  std::vector<uint8_t> sig = pk->sign(Caligo::Tls13SignatureScheme::rsa_pss_pss_sha256, message);
  bool sigOk = pubkey->validateSignature(Caligo::Tls13SignatureScheme::rsa_pss_pss_sha256, message, sig);
  REQUIRE(sigOk);
}
*/
