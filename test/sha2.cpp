...
TEST_CASE("SHA sanity check", "[SHA]") {
  ...
  std::vector<uint8_t> grape_h = SHA256(std::span<uint8_t>((unsigned char*)grape, 5));
  REQUIRE(grape_h == grape_hash);


  std::vector<uint8_t> s1 = SHA256();
