#include <catch/catch.hpp>
#include <caligo/hkdf.h>
#include <caligo/sha2.h>

struct MyCipher {
  static constexpr size_t size = 16;
  static constexpr size_t ivsize = 12;
  static constexpr size_t keysize = 16;
};

TEST_CASE("hkdf single test vector", "[HKDF]") {
  s2::vector<uint8_t> shared = { 0xdf, 0x4a, 0x29, 0x1b, 0xaa, 0x1e, 0xb7, 0xcf, 0xa6, 0x93, 0x4b, 0x29, 0xb4, 0x74, 0xba, 0xad, 0x26, 0x97, 0xe2, 0x9f, 0x1f, 0x92, 0x0d, 0xcc, 0x77, 0xc8, 0xa0, 0xa0, 0x88, 0x44, 0x76, 0x24 };
  s2::vector<uint8_t> handshake_secret = { 0xfb, 0x9f, 0xc8, 0x06, 0x89, 0xb3, 0xa5, 0xd0, 0x2c, 0x33, 0x24, 0x3b, 0xf6, 0x9a, 0x1b, 0x1b, 0x20, 0x70, 0x55, 0x88, 0xa7, 0x94, 0x30, 0x4a, 0x6e, 0x71, 0x20, 0x15, 0x5e, 0xdf, 0x14, 0x9a };

  auto hs = HKDF_HandshakeSecret<SHA256>(shared);
  CHECK(hs.data == handshake_secret);

  s2::vector<uint8_t> helloHash = { 0xda, 0x75, 0xce, 0x11, 0x39, 0xac, 0x80, 0xda, 0xe4, 0x04, 0x4d, 0xa9, 0x32, 0x35, 0x0c, 0xf6, 0x5c, 0x97, 0xcc, 0xc9, 0xe3, 0x3f, 0x1e, 0x6f, 0x7d, 0x2d, 0x4b, 0x18, 0xb7, 0x36, 0xff, 0xd5 };

  s2::vector<uint8_t> handshake_c_traffic = { 0xff, 0x0e, 0x5b, 0x96, 0x52, 0x91, 0xc6, 0x08, 0xc1, 0xe8, 0xcd, 0x26, 0x7e, 0xef, 0xc0, 0xaf, 0xcc, 0x5e, 0x98, 0xa2, 0x78, 0x63, 0x73, 0xf0, 0xdb, 0x47, 0xb0, 0x47, 0x86, 0xd7, 0x2a, 0xea };
  s2::vector c_tr = HKDF_Expand_Label<SHA256>(handshake_secret, "c hs traffic", helloHash, 32);
  CHECK(c_tr == handshake_c_traffic);
  s2::vector<uint8_t> handshake_s_traffic = { 0xa2, 0x06, 0x72, 0x65, 0xe7, 0xf0, 0x65, 0x2a, 0x92, 0x3d, 0x5d, 0x72, 0xab, 0x04, 0x67, 0xc4, 0x61, 0x32, 0xee, 0xb9, 0x68, 0xb6, 0xa3, 0x2d, 0x31, 0x1c, 0x80, 0x58, 0x68, 0x54, 0x88, 0x14 };
  s2::vector s_tr = HKDF_Expand_Label<SHA256>(handshake_secret, "s hs traffic", helloHash, 32);
  CHECK(s_tr == handshake_s_traffic);

  s2::vector<uint8_t> handshake_c_key = { 0x71, 0x54, 0xf3, 0x14, 0xe6, 0xbe, 0x7d, 0xc0, 0x08, 0xdf, 0x2c, 0x83, 0x2b, 0xaa, 0x1d, 0x39 };
  s2::vector<uint8_t> handshake_s_key = { 0x84, 0x47, 0x80, 0xa7, 0xac, 0xad, 0x9f, 0x98, 0x0f, 0xa2, 0x5c, 0x11, 0x4e, 0x43, 0x40, 0x2a };
  s2::vector<uint8_t> handshake_c_iv = { 0x71, 0xab, 0xc2, 0xca, 0xe4, 0xc6, 0x99, 0xd4, 0x7c, 0x60, 0x02, 0x68 };
  s2::vector<uint8_t> handshake_s_iv = { 0x4c, 0x04, 0x2d, 0xdc, 0x12, 0x0a, 0x38, 0xd1, 0x41, 0x7f, 0xc8, 0x15 };

  key_iv_pair<MyCipher> server = hs.get_key_iv<MyCipher>(helloHash, false, true);
  key_iv_pair<MyCipher> client = hs.get_key_iv<MyCipher>(helloHash, true, true);

  CHECK(server.key == handshake_s_key);
  CHECK(server.iv == handshake_s_iv);
  CHECK(client.key == handshake_c_key);
  CHECK(client.iv == handshake_c_iv);

	
  s2::vector<uint8_t> hshash = { 0x22, 0x84, 0x4b, 0x93, 0x0e, 0x5e, 0x0a, 0x59, 0xa0, 0x9d, 0x5a, 0xc3, 0x5f, 0xc0, 0x32, 0xfc, 0x91, 0x16, 0x3b, 0x19, 0x38, 0x74, 0xa2, 0x65, 0x23, 0x6e, 0x56, 0x80, 0x77, 0x37, 0x8d, 0x8b };
  auto ms = HKDF_MasterSecret<SHA256>(hs);
  key_iv_pair<MyCipher> serverM = ms.get_key_iv<MyCipher>(hshash, false);
  key_iv_pair<MyCipher> clientM = ms.get_key_iv<MyCipher>(hshash, true);



  s2::vector<uint8_t> session_c_key = { 0x49, 0x13, 0x4b, 0x95, 0x32, 0x8f, 0x27, 0x9f, 0x01, 0x83, 0x86, 0x05, 0x89, 0xac, 0x67, 0x07 };
  s2::vector<uint8_t> session_s_key = { 0x0b, 0x6d, 0x22, 0xc8, 0xff, 0x68, 0x09, 0x7e, 0xa8, 0x71, 0xc6, 0x72, 0x07, 0x37, 0x73, 0xbf };
  s2::vector<uint8_t> session_c_iv = { 0xbc, 0x4d, 0xd5, 0xf7, 0xb9, 0x8a, 0xcf, 0xf8, 0x54, 0x66, 0x26, 0x1d }; 
  s2::vector<uint8_t> session_s_iv = { 0x1b, 0x13, 0xdd, 0x9f, 0x8d, 0x8f, 0x17, 0x09, 0x1d, 0x34, 0xb3, 0x49 };
  CHECK(serverM.key == session_s_key);
  CHECK(serverM.iv == session_s_iv);
  CHECK(clientM.key == session_c_key);
  CHECK(clientM.iv == session_c_iv);
}

TEST_CASE("hkdf finished calculation (Client)", "[HKDF]") {
  s2::vector<uint8_t> messages = {
     ,

  };
  SHA256 hashObj(messages);
  s2::vector<uint8_t> serverFinHash = hashObj;

  s2::vector<uint8_t> sht_secret = { 0xa2, 0x06, 0x72, 0x65, 0xe7, 0xf0, 0x65, 0x2a, 0x92, 0x3d, 0x5d, 0x72, 0xab, 0x04, 0x67, 0xc4, 0x61, 0x32, 0xee, 0xb9, 0x68, 0xb6, 0xa3, 0x2d, 0x31, 0x1c, 0x80, 0x58, 0x68, 0x54, 0x88, 0x14 };
  auto s_fin_key = HKDF_Expand_Label<SHA256>(sht_secret, "finished", {}, 32);
  s2::vector<uint8_t> shash = HMAC<SHA256>(serverFinHash, s_fin_key);
  s2::vector<uint8_t> serverDigest = { 0xea, 0x6e, 0xe1, 0x76, 0xdc, 0xcc, 0x4a, 0xf1, 0x85, 0x9e, 0x9e, 0x4e, 0x93, 0xf7, 0x97, 0xea, 0xc9, 0xa7, 0x8c, 0xe4, 0x39, 0x30, 0x1e, 0x35, 0x27, 0x5a, 0xd4, 0x3f, 0x3c, 0xdd, 0xbd, 0xe3 };
  REQUIRE(serverDigest == shash);

  hashObj.add(s2::vector<uint8_t>{ 0x14, 0x00, 0x00, 0x20 });
  hashObj.add(serverDigest);

  s2::vector<uint8_t> cht_secret = { 0xff, 0x0e, 0x5b, 0x96, 0x52, 0x91, 0xc6, 0x08, 0xc1, 0xe8, 0xcd, 0x26, 0x7e, 0xef, 0xc0, 0xaf, 0xcc, 0x5e, 0x98, 0xa2, 0x78, 0x63, 0x73, 0xf0, 0xdb, 0x47, 0xb0, 0x47, 0x86, 0xd7, 0x2a, 0xea };
  s2::vector<uint8_t> fin_hash = { 0x22, 0x84, 0x4b, 0x93, 0x0e, 0x5e, 0x0a, 0x59, 0xa0, 0x9d, 0x5a, 0xc3, 0x5f, 0xc0, 0x32, 0xfc, 0x91, 0x16, 0x3b, 0x19, 0x38, 0x74, 0xa2, 0x65, 0x23, 0x6e, 0x56, 0x80, 0x77, 0x37, 0x8d, 0x8b };
  s2::vector<uint8_t> clientFinHash = hashObj;
  auto fin_key = HKDF_Expand_Label<SHA256>(cht_secret, "finished", {}, 32);
  s2::vector<uint8_t> hash = HMAC<SHA256>(fin_hash, fin_key);
  s2::vector<uint8_t> digest = { 0x97, 0x60, 0x17, 0xa7, 0x7a, 0xe4, 0x7f, 0x16, 0x58, 0xe2, 0x8f, 0x70, 0x85, 0xfe, 0x37, 0xd1, 0x49, 0xd1, 0xe9, 0xc9, 0x1f, 0x56, 0xe1, 0xae, 0xbb, 0xe0, 0xc6, 0xbb, 0x05, 0x4b, 0xd9, 0x2b };
  REQUIRE(fin_hash == clientFinHash);
  REQUIRE(hash == digest);
}




